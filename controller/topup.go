...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if rebateErr := model.AddTopupRebateForInviter(model.DB, topUp.UserId, quotaToAdd); rebateErr != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值返利失败 trade_no=%s user_id=%d error=%q", topUp.TradeNo, topUp.UserId, rebateErr.Error()))
			}
